...
def test_new_user(mocker):
    ...
    assert write_mock.call_args_list == [call('nicknames', 'nickname', {'user': 'user_id'}),
...
def test_update_nickname(mocker):
    ...
    assert write_mock.call_args == call('nicknames', 'nickname_new', {'user': 'user_id'})
    assert update_mock.call_args == call('public_users', 'user_id', {'nickname': 'nickname_new'})
...
def test_delete_user(mocker):
    ...
    assert delete_mock.call_args_list == [call('nicknames', 'nickname'), call('public_users', 'user_id')]
