...
def test_rollover_individuals__roll_amt(setup):
    ...
    assert (
        len(list(f.get_collection("individuals").stream())) == individual_amt + roll_amt
    )
...
def test_rollover_individuals__documents(setup):
    ...
    for individual_doc in d.query_individuals("year", "==", 2013).stream():
        ...
        assert (
            "last_phenophase" not in individual_doc.to_dict()
        ), individual_doc.to_dict()
        assert (
            "last_observation_date" not in individual_doc.to_dict()
        ), individual_doc.to_dict()
