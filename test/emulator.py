...
def start(xprocess, name):
    class EmulatorClass(ProcessStarter):
        args = [_get_gcloud_cmd(), 'beta', 'emulators', 'firestore', 'start', '--project', 'test', '--host-port',
                'localhost:8001']
    ...
    phenoback.utils.firestore._db = firestore.Client(project="test", credentials=credentials)
