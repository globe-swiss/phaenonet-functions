...
if [ "$1" = 'all' ] || [ "$1" = 'ts' ]; then
  ...
  gcloud functions deploy process_ts_definitions --project $PROJECT --entry-point process_document_ts_write --runtime python37 --trigger-resource "projects/$PROJECT/databases/(default)/documents/definitions/{document}" --trigger-event providers/cloud.firestore/eventTypes/document.write --timeout 540s --region europe-west1 --quiet --env-vars-file env.$PROJECT.yaml & sleep 2
fi
